...
template<typename KeyT>
bool LeafNode<KeyT>::IsValid(size_t entryNumber) const {
	size_t offset = CalculateEntryOffset(entryNumber) + sizeof(KeyT);
	return 0 == GetValueByOffset<std::uint8_t>(m_bytes, offset);
}

template<typename KeyT>
void LeafNode<KeyT>::MarkInvalid(size_t entryNumber) {
	size_t offset = CalculateEntryOffset(entryNumber) + sizeof(KeyT);

...
namespace parser_tests
{
	...
	TEST(SelectTests, SimpleJoinTest) {
		...
		ASSERT_NE(dynamic_cast<JoinStatement *>(statement.get()), nullptr);
	}
	...
	TEST(SelectTests, FirstPartJoinMultiWhereTest) {
		auto statement = SQLParser::Instance().ParseStatement("select table1.col2, table2.col2 from table1 join table2 on table1.col1 = table2.col2 where table1.col1 = \"Do you want use a haskell?\" and table2.col2 < 5;");
		ASSERT_NE(dynamic_cast<JoinStatement*>(statement.get()), nullptr);
	}

	TEST(SelectTests, SecondPartJoinMultiWhereTest) {
		auto statement = SQLParser::Instance().ParseStatement("select col1, t2.col2 from t1 join t2 on t1.col1 = t2.field2 where t1.col2 > 4 and t2.field3 = \"Are you feeling a pain?\";");
		ASSERT_NE(dynamic_cast<JoinStatement *>(statement.get()), nullptr);
	}
}

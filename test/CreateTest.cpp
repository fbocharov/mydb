...
namespace parser_tests
{
    TEST(CreateTableTests, IntFieldTypeTest) {
        ...
        ASSERT_EQ(field1Column.type, FieldType::INT);
    }
    ...
    TEST(CreateTableTests, DoubleFieldTypeTest) {
        ...
        ASSERT_EQ(field1Column.type, FieldType::DOUBLE);
        ASSERT_EQ(columns[1].type, FieldType::DOUBLE);
    }
    ...
    TEST(CreateTableTests, VarcharTypeTest) {
        ...
        auto columns = createStatement->GetColumns();
        ASSERT_EQ(columns.size(), 1);
        ASSERT_EQ(columns[0].type, FieldType::VARCHAR);
    }

...
TEST(ParserCommonTests, WrongCaseTypeNameTest) {
    ASSERT_THROW(SQLParser::Instance().ParseStatement("create table mytable(field2 Int"), SQLParserException);
}
...
TEST(ParserCommonTests, WrongStatementNameTest) {
    ASSERT_THROW(SQLParser::Instance().ParseStatement("create rable mytable(field1);"), SQLParserException);
}
